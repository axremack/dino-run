/*class Balle extends Sprite {
    ...
    public bouger(){
        ...
        for(let i :number=0;i<briques.length && !touche;i++){
            if(briques[i] != null && Sprite.collision(this.getCercle(),briques[i].getRectangle())){
                this.scene_.retirer(briques[i]);
                this.vy_=-1*this.vy_;
                briques[i]=null;
                touche=true;
                this.scene_.compteur_++;
                this.scene_.score_.getBalise().innerHTML=""+this.scene_.compteur_;
            }
        }
